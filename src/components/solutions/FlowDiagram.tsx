import type { FlowStage, SolutionFlow } from "@/lib/solutions/types";

/**
 * A solution's agent perimeter as a top-down flow diagram: inbound demand →
 * three agent stages → one unified record → across the boundary into the
 * client's own core system. A left-hand dashed bracket draws the re-engagement
 * loop back up into stage 01 (lg and up; the same information is spelled out in
 * the caption below the stages at every width).
 *
 ...
 * SolutionView's section reveal stagger the blocks; under reduced motion that
 ...
 */
export default function FlowDiagram({ flow }: { flow: SolutionFlow }) {
  return (
    <figure className="mt-14 md:mt-16">
      <figcaption className="sr-only">{flow.caption}</figcaption>
      ...
      {/* ── the agent stages, wrapped by the re-engagement bracket ───────── */}
      ...
      <div className="li-reveal border border-accent bg-surface p-5 md:p-6 lg:ml-16">
        <div className="grid gap-3 md:grid-cols-12 md:items-center md:gap-8">
          <div className="md:col-span-4">
            <span className="font-mono text-[10px] tracking-[0.3em] text-accent">{flow.record.eyebrow}</span>
            <h3 className="mt-2 font-display text-xl font-extrabold uppercase tracking-wide text-ink md:text-2xl">
              {flow.record.title}
            </h3>
          </div>
          <p className="text-[13px] leading-relaxed text-ink-2 md:col-span-8">{flow.record.detail}</p>
        </div>
      </div>
      ...
      {/* ── boundary: past here it is the client's system, not an agent ──── */}
